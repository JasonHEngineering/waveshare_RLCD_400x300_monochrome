...
#define RMS_THRESHOLD    1000 //300 for living room, but I will use high value for avoiding accidental triggers 
#define TRIGGER_FRAMES   3

#define CONFIG_BUFFER_SIZE 32
#define openai_key_CONFIG_BUFFER_SIZE 200

CustomSDPort *sdcardPort = NULL;
char config_buffer[4096] = {0};
char ssid1[CONFIG_BUFFER_SIZE];
char pass1[CONFIG_BUFFER_SIZE];
char ssid2[CONFIG_BUFFER_SIZE];
char pass2[CONFIG_BUFFER_SIZE];
char LTA_mall_apiKey[CONFIG_BUFFER_SIZE];
char openai_key[openai_key_CONFIG_BUFFER_SIZE];
char busStopCode_one[CONFIG_BUFFER_SIZE];
char busStopCode_two[CONFIG_BUFFER_SIZE];
char bus_stop_name_one[CONFIG_BUFFER_SIZE];
char bus_stop_name_two[CONFIG_BUFFER_SIZE];
...
const char* temp_RH_station_ = "S109"; // S109 is for "Ang Mo Kio Avenue 5"
...
void updateBusArrival_one() {
  ...
  http1.begin(url);
  http1.addHeader("AccountKey", LTA_mall_apiKey);
...
void connecting_to_wifi_screen() {
...
void parseConfig(char *data)
{
  ...
  while (line != NULL)
  {
    ...
    if (eq)
    {
      *eq = '\0';           // split key and value
      ...
      if (strcmp(key, "wifi_ssid1") == 0)
      {
        strncpy(ssid1, value, CONFIG_BUFFER_SIZE - 1);
        ssid1[CONFIG_BUFFER_SIZE - 1] = '\0';
      }
      else if (strcmp(key, "wifi_pass1") == 0)
      {
        strncpy(pass1, value, CONFIG_BUFFER_SIZE - 1);
        pass1[CONFIG_BUFFER_SIZE - 1] = '\0';
      }
      else if (strcmp(key, "wifi_ssid2") == 0)
      {
        strncpy(ssid2, value, CONFIG_BUFFER_SIZE - 1);
        ssid2[CONFIG_BUFFER_SIZE - 1] = '\0';
      }
      else if (strcmp(key, "wifi_pass2") == 0)
      {
        strncpy(pass2, value, CONFIG_BUFFER_SIZE - 1);
        pass2[CONFIG_BUFFER_SIZE - 1] = '\0';
      }
      else if (strcmp(key, "LTA_mall_apiKey") == 0)
      {
        strncpy(LTA_mall_apiKey, value, CONFIG_BUFFER_SIZE - 1);
        LTA_mall_apiKey[CONFIG_BUFFER_SIZE - 1] = '\0';
      }
      else if (strcmp(key, "openai_key") == 0)
      {
        strncpy(openai_key, value, openai_key_CONFIG_BUFFER_SIZE - 1);
        openai_key[openai_key_CONFIG_BUFFER_SIZE - 1] = '\0';
      }
      else if (strcmp(key, "busStopCode_one") == 0)
      {
        strncpy(busStopCode_one, value, CONFIG_BUFFER_SIZE - 1);
        busStopCode_one[CONFIG_BUFFER_SIZE - 1] = '\0';
      }
      else if (strcmp(key, "busStopCode_two") == 0)
      {
        strncpy(busStopCode_two, value, CONFIG_BUFFER_SIZE - 1);
        busStopCode_two[CONFIG_BUFFER_SIZE - 1] = '\0';
      }
      else if (strcmp(key, "bus_stop_name_one") == 0)
      {
        strncpy(bus_stop_name_one, value, CONFIG_BUFFER_SIZE - 1);
        bus_stop_name_one[CONFIG_BUFFER_SIZE - 1] = '\0';
      }
      else if (strcmp(key, "bus_stop_name_two") == 0)
      {
        strncpy(bus_stop_name_two, value, CONFIG_BUFFER_SIZE - 1);
        bus_stop_name_two[CONFIG_BUFFER_SIZE - 1] = '\0';
      }
    }
  ...
  Serial.println(LTA_mall_apiKey);
  Serial.println(openai_key);
  Serial.println(busStopCode_one);
  Serial.println(busStopCode_two);
  Serial.println(bus_stop_name_one);
  Serial.println(bus_stop_name_two);

}

void read_sd_card_config() {

  sdcardPort = new CustomSDPort("/sdcard");

  sdcardPort->SDPort_ReadFile("/sdcard/config.txt",(uint8_t *)config_buffer,NULL);
  vTaskDelay(pdMS_TO_TICKS(200));

  parseConfig(config_buffer);
  vTaskDelay(pdMS_TO_TICKS(200));
...
void setup() {
  ...
  // show booting up screen
  connecting_to_wifi_screen();
