...
// manually added here for displaying custom .bin file
void DisplayPort::RLCD_DisplayRaw(const uint8_t* data) {
  RLCD_SendCommand(0x2A);
  RLCD_SendData(0x12);
  RLCD_SendData(0x2A);
  ...
  RLCD_Sendbuffera((uint8_t*)data, DisplayLen);
}

// manually added here for displaying custom .bin file
void DisplayPort::RLCD_LoadFullBuffer(const uint8_t* data)
{
    memcpy(DispBuffer, data, DisplayLen);
}
